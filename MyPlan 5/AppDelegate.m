...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    self.weekEdit = [[WeekEdit alloc] init];
    self.weekMenu.delegate = self;
    self.weekView.delegate = self;
    self.weekViewNav = [[UINavigationController alloc] initWithRootViewController:self.weekView];
    self.weekViewNav.navigationBar.opaque = NO;
    self.weekViewNav.navigationBar.barStyle = UIBarStyleBlackTranslucent;
    
//    MPNavigationBar *bar = [[MPNavigationBar alloc] initWithFrame:self.weekViewNav.navigationBar.frame];
//    bar.tintColor = [UIColor blackColor];
//    bar.barStyle = UIBarStyleBlackTranslucent;
//    [self.weekViewNav setValue:bar forKey:@"navigationBar"];
    
    self.revealSideViewController = [[SWRevealViewController alloc] initWithRearViewController:self.weekMenu frontViewController:self.weekViewNav];
    self.revealSideViewController.rearViewRevealWidth = 250;
    self.revealSideViewController.rearViewRevealOverdraw = 0;
    self.revealSideViewController.bounceBackOnOverdraw = YES;
    self.revealSideViewController.stableDragOnOverdraw = YES;
    self.revealSideViewController.delegate = self;
    [self.weekMenu viewDidAppear:YES];
...
//            [self.weekView presentViewController:self.update animated:YES completion:NULL];
    ...
    if ([appData showAdMessage] && appData.AdFree == NO) {
        [self showAdMessage];
    }
