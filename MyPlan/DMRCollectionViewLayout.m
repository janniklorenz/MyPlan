...
-(id)init
{
    ...
    if (self)
    {
        [self setItemOffset:UIOffsetMake(2.0, 2.0)];
    }
    return self;
}
...
-(void)prepareLayout
{
    ...
    for (NSUInteger index = 0; index < numberOfItems; index++) {
        ...
        float a = (float)((arc4random() % 2)+1)/4;
        CGSize itemSize = [self sizeForItemWithColumnX:a y:a];
        
        // Set max row height
        if (itemSize.height > rowHeight) rowHeight = itemSize.height;
        ...
        if (column == numberOfColumnsInRow) {
            if (xOffset > contentWidth) contentWidth = xOffset;
...
-(CGSize)sizeForItemWithColumnX:(float)x y:(float)y
{
    return CGSizeMake(self.collectionView.bounds.size.width/5-_itemOffset.horizontal, self.collectionView.bounds.size.height/10);
